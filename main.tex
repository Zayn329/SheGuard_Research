...
\begin{tikzpicture}[
    ...
    card/.style={draw, rounded corners=2pt, align=left, inner sep=3pt, fill=white, font=\sffamily\tiny},
    ...
    arr/.style={->, >=stealth, line width=0.8pt, color=blue!80!black},
    darr/.style={->, >=stealth, line width=0.8pt, dashed, color=gray!80!black},
    carr/.style={->, >=stealth, line width=0.8pt, dotted, color=teal!80!black},
    lbl/.style={font=\sffamily\tiny\bfseries, fill=white, inner sep=1pt}
]

% Define Palette
\colorlet{sensbg}{blue!5}
\colorlet{sensborder}{blue!60!black}
\colorlet{intelbg}{green!5}
\colorlet{intelborder}{green!60!black}
\colorlet{commbg}{violet!5}
\colorlet{commborder}{violet!60!black}
\colorlet{recipbg}{violet!10}
\colorlet{recipborder}{violet!70!black}
\colorlet{evidbg}{orange!5}
\colorlet{evidborder}{orange!80!black}
\colorlet{backbg}{blue!10}
\colorlet{backborder}{blue!80!black}
\colorlet{aibg}{purple!5}
\colorlet{aiporder}{purple!80!black}
...
\node [card, text width=3.2cm] (audio) {
    \textbf{Audio Input Processing}\\
    \textbullet\ Continuous 16\,kHz PCM\\
    \textbullet\ 10s Ring Buffer
};

\node [card, below=0.25cm of audio, text width=3.2cm] (keyword) {
    \textbf{Keyword Detection}\\
    \textbullet\ Configurable Triggers\\
    \textbullet\ Speech Commands Model
};

\node [card, below=0.25cm of keyword, text width=3.2cm] (motion) {
    \textbf{Motion Detection}\\
    \textbullet\ Accelerometer Sampling\\
    \textbullet\ Impact \& Fall Anomaly
};

\begin{pgfonlayer}{background}
\node [sec, draw=sensborder, fill=sensbg, fit={(audio) (keyword) (motion)}, label={[font=\sffamily\bfseries\tiny, text=sensborder, anchor=north]north:\textbf{SENSING LAYER (On-device)}}] (senslayer) {};
\end{pgfonlayer}

% --- COLUMN 2: ON-DEVICE INTELLIGENCE LAYER ---
\node [card, right=0.6cm of audio, text width=4.8cm] (preproc) {
    \textbf{Signal Preprocessing}\\
    \textbullet\ Noise Reduction \& Spectrogram Feature Slicing
};

\node [card, below=0.2cm of preproc, text width=4.8cm] (fusion) {
    \textbf{Signal Fusion Engine}\\
    Governing Fusion Rule:\\
    \scalebox{0.82}{$K_t \lor (A_t \ge \theta_A \land M_t' \ge \theta_M \land |t_A - t_M| \le \Delta t_{\mathrm{sync}})$}
};

% Incident State Machine Nodes inside Column 2
\node [box, draw=intelborder, fill=white, below=0.2cm of fusion, xshift=-1.8cm, text width=1.1cm] (st_idle) {IDLE};
\node [box, draw=intelborder, fill=white, right=0.15cm of st_idle, text width=1.3cm] (st_mon) {MONITORING};
\node [box, draw=intelborder, fill=white, right=0.15cm of st_mon, text width=1.3cm] (st_susp) {SUSPICIOUS};

\node [box, draw=intelborder, fill=white, below=0.25cm of st_idle, xshift=0.3cm, text width=1.4cm] (st_cand) {CANDIDATE};

% Alert / Confirmation Nodes
\node [box, draw=red!80!black, fill=red!15, right=0.2cm of st_cand, text width=1.8cm] (st_pend) {\textbf{PENDING}\\ \textbf{CONFIRMATION}\\ (6s Timeout)};

\node [box, draw=gray!60, fill=gray!10, below=0.2cm of st_pend, xshift=-1.1cm, text width=1.3cm] (st_canc) {CANCELLED\\ (PIN)};

\node [box, draw=red!90!black, fill=red!80!black, text=white, right=0.25cm of st_pend, text width=1.3cm] (st_act) {\textbf{ACTIVE}\\ \textbf{INCIDENT}};

\node [box, draw=intelborder, fill=white, right=0.2cm of st_act, text width=1.1cm] (st_seal) {SEALED};

\begin{pgfonlayer}{background}
\node [sec, draw=intelborder, fill=intelbg, fit={(preproc) (fusion) (st_idle) (st_susp) (st_canc) (st_seal)}, label={[font=\sffamily\bfseries\tiny, text=intelborder, anchor=north]north:\textbf{ON-DEVICE INTELLIGENCE LAYER}}] (intellayer) {};
\end{pgfonlayer}

% --- COLUMN 3: COMMUNICATION LAYER ---
\node [card, right=0.6cm of preproc, text width=3.4cm] (nearby) {
    \textbf{Nearby Connections}\\
    \textbullet\ \texttt{P2P\_CLUSTER} Strategy\\
    \textbullet\ BLE \& Wi-Fi Direct Mesh\\
    \textbullet\ Multi-hop Peer Relay
};

\node [card, below=0.3cm of nearby, text width=3.4cm] (sms) {
    \textbf{Cellular SMS Fallback}\\
    \textbullet\ Direct Cellular SMS\\
    \textbullet\ Encrypted GSM Payload\\
    \textbullet\ Location Coordinates
};

\begin{pgfonlayer}{background}
\node [sec, draw=commborder, fill=commbg, fit={(nearby) (sms)}, label={[font=\sffamily\bfseries\tiny, text=commborder, anchor=north]north:\textbf{COMMUNICATION LAYER (Offline-first)}}] (commlayer) {};
\end{pgfonlayer}

% --- COLUMN 4: AUTHORIZED RECIPIENTS ---
\node [card, right=0.6cm of nearby, text width=3.2cm] (contacts) {
    \textbf{Notify Circle}\\
    \textbullet\ Emergency Contacts\\
    \textbullet\ Nominated Guardians
};

\node [card, below=0.2cm of contacts, text width=3.2cm] (peers) {
    \textbf{Nearby Devices}\\
    \textbullet\ SheGuard Mesh Peers\\
    \textbullet\ Opportunistic Relays
};

\node [card, below=0.2cm of peers, text width=3.2cm] (authorities) {
    \textbf{Authorized Support}\\
    \textbullet\ First Responders\\
    \textbullet\ Police / Medical
};

\begin{pgfonlayer}{background}
\node [sec, draw=recipborder, fill=recipbg, fit={(contacts) (peers) (authorities)}, label={[font=\sffamily\bfseries\tiny, text=recipborder, anchor=north]north:\textbf{AUTHORIZED RECIPIENTS}}] (reciplayer) {};
\end{pgfonlayer}

% TOP CONTAINER BOUNDARY BOX
\begin{pgfonlayer}{background}
\node [draw=green!60!black, fill=green!2!white, rounded corners=6pt, line width=1.2pt, dashed, fit={(senslayer) (intellayer) (commlayer) (reciplayer)}, label={[font=\sffamily\bfseries\small, text=green!50!black, anchor=north]north:\textbf{Core local safety path -- Operates without Internet, backend, or external AI provider}}] (topcontainer) {};
\end{pgfonlayer}
...
\node [card, below=1.0cm of senslayer.south west, anchor=north west, text width=3.2cm] (ev_cap) {
    \textbf{Evidence Capture}\\
    \textbullet\ Audio Pre-roll Ring Buffer\\
    \textbullet\ GPS Coordinates \& Sensor Logs\\
    \textbullet\ High-Priority Media Capture
};

\node [card, right=0.4cm of ev_cap, text width=3.8cm] (ev_kms) {
    \textbf{Encryption \& Key Management}\\
    \textbullet\ AES-256-GCM Payload Encryption\\
    \textbullet\ Per-Incident DEK Key Hierarchy\\
    \textbullet\ Android Keystore Wrapped KEK
};

\node [card, right=0.4cm of ev_kms, text width=3.8cm] (ev_seal) {
    \textbf{Integrity \& Sealing}\\
    \textbullet\ SHA-256 Merkle Tree Hash\\
    \textbullet\ Root Digest $R_{\mathrm{Merkle}}$ Sealing\\
    \textbullet\ ECDSA Hardware-Signed Manifest
};

\node [card, right=0.4cm of ev_seal, text width=3.2cm] (ev_store) {
    \textbf{Local Storage}\\
    \textbullet\ Encrypted Room SQLite\\
    \textbullet\ Hardware-Backed Vault\\
    \textbullet\ App-Private Sandboxing
};

\begin{pgfonlayer}{background}
\node [sec, draw=evidborder, fill=evidbg, fit={(ev_cap) (ev_kms) (ev_seal) (ev_store)}, label={[font=\sffamily\bfseries\small, text=evidborder, anchor=north]north:\textbf{EVIDENCE CAPTURE AND SECURITY LAYER}}] (evidlayer) {};
\end{pgfonlayer}

% BESIDE EVIDENCE LAYER: OPTIONAL BACKEND
\node [sec, draw=backborder, fill=backbg, right=0.5cm of evidlayer, text width=2.8cm, minimum height=2.2cm, align=center] (backend) {
    \textbf{Optional Backend Services}\\[2pt]
    \textbullet\ Encrypted Cloud Storage\\
    \textbullet\ Asynchronous Sync\\
    \textbullet\ WORM Evidence Vault
};
...
\node [card, below=0.9cm of evidlayer.south west, anchor=north west, text width=4.0cm] (ai_sum) {
    \textbf{Incident Summarization}\\
    \textbullet\ Multimodal Timeline Assembly\\
    \textbullet\ Contextual Log Structuring
};

\node [card, right=0.5cm of ai_sum, text width=4.0cm] (ai_fir) {
    \textbf{FIR Drafting}\\
    \textbullet\ Structured Draft Generation\\
    \textbullet\ Legal Format Alignment
};

\node [card, right=0.5cm of ai_fir, text width=4.0cm] (ai_leg) {
    \textbf{Legal / Procedural Info}\\
    \textbullet\ Contextual Rights Guidance\\
    \textbullet\ First Responder Orientation
};

% AI Footer Banner
\node [box, draw=purple!80!black, fill=purple!20, below=0.2cm of ai_fir, text width=13.0cm, xshift=-0.2cm] (ai_footer) {
    \textbf{AI cannot detect, decide, escalate, capture, seal, or file $\vert$ Not in Emergency Decision Path}
};

\begin{pgfonlayer}{background}
\node [sec, draw=aiporder, fill=aibg, fit={(ai_sum) (ai_fir) (ai_leg) (ai_footer)}, label={[font=\sffamily\bfseries\small, text=aiporder, anchor=north]north:\textbf{POST-INCIDENT AI ASSISTANCE (Human-in-the-Loop; non-safety-critical)}}] (ailayer) {};
\end{pgfonlayer}


% =========================================================================
% BOTTOM RIGHT: ARROW LEGEND
% =========================================================================

\node [box, draw=gray!60, fill=white, right=0.5cm of ailayer, text width=2.8cm, align=left] (legend) {
    \textbf{Legend:}\\[2pt]
    \tikz\draw[arr](0,0)--(0.4,0);\, Local Data Flow\\[2pt]
    \tikz\draw[darr](0,0)--(0.4,0);\, Optional / Network\\[2pt]
    \tikz\draw[carr](0,0)--(0.4,0);\, Control / Ack Flow
};
...
\draw [arr] (audio.east) -- (preproc.west);
\draw [arr] (keyword.east) -- (preproc.west);
\draw [arr] (motion.east) -- (fusion.west);

% Preproc -> Fusion
\draw [arr] (preproc.south) -- (fusion.north);

% Fusion -> State Machine
\draw [arr] (fusion.south) -- ++(0,-0.1) -| (st_idle.north);

% State Machine Internal Transitions
...
% Active Incident -> Comm Layer
\draw [arr] (st_act.north) -- ++(0,0.4) -| (nearby.west);
\draw [arr] (st_act.north) -- ++(0,0.4) -| (sms.west);

% Comm Layer -> Recipients
\draw [arr] (nearby.east) -- (peers.west);
\draw [arr] (nearby.east) -- (authorities.west);
\draw [arr] (sms.east) -- (contacts.west);

% Active Incident -> Evidence Capture
\draw [arr] (st_act.south) -- ++(0,-0.5) -| (ev_cap.north);

% Evidence Internal Pipeline
\draw [arr] (ev_cap.east) -- (ev_kms.west);
\draw [arr] (ev_kms.east) -- (ev_seal.west);
\draw [arr] (ev_seal.east) -- (ev_store.west);

% Evidence -> Backend
\draw [darr] (ev_store.east) -- (backend.west);

% Evidence -> AI Layer
\draw [carr] (ev_store.south) -- ++(0,-0.4) -| (ai_sum.north);
